...
export const ENROLLMENT_STATUS_SET: ReadonlySet<EnrollmentStatus> =
  new Set<EnrollmentStatus>(ENROLLMENT_STATUSES);
 ...
 */
export const LEGACY_STATUS_ALIASES: Readonly<Record<string, EnrollmentStatus>> = {
  CANCELLED: "EXITED",
  ABANDONED: "EXITED",
};

/** Type guard — accepts unknown, narrows to EnrollmentStatus. */
export function isCanonicalEnrollmentStatus(value: unknown): value is EnrollmentStatus {
  return typeof value === "string" && ENROLLMENT_STATUS_SET.has(value as EnrollmentStatus);
}
...
export function normalizeEnrollmentStatus(value: unknown): EnrollmentStatus | null {
  ...
  if (ENROLLMENT_STATUS_SET.has(upper as EnrollmentStatus)) return upper as EnrollmentStatus;
